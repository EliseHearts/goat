...
class NewVisitorTest(StaticLiveServerTestCase):
    ...
    def test_can_start_a_list_for_one_user(self):
        ...
        # another item. She enters "Use peacock feathers to make 
        # a fly" (Edith is very methodical)
        inputbox = self.browser.find_element_by_id('id_new_item')
        inputbox.send_keys('Use peacock feathers to make a fly')
        inputbox.send_keys(Keys.ENTER)

        # The page updates again, and now shows both items in her 
        # list
        self.wait_for_row_in_list_table('1: Buy peacock feathers')
    ...
    def test_multiple_users_can_start_lists_at_different_urls(self):
        ...
        # Now a new user, Francis, comes along to the new site.

        ## We use a new browser session to make sure that no 
        ## information of Edith's is coming through from cookies 
        ## etc
        self.browser.quit()
        self.browser = webdriver.Firefox()
        ...
        # Francis starts a new ilst by entering a new item. He
    ...
    def test_layout_and_styling(self):
        ...
        self.assertAlmostEqual(
            inputbox.location['x'] + inputbox.size['width']/2, size["width"]/2, delta=10
        )

        # She starts a new list and sees the input is
        # nicely centered there too
        inputbox.send_keys('derp')
        inputbox.send_keys(Keys.ENTER)
        self.wait_for_row_in_list_table('1: derp')
        inputbox = self.browser.find_element_by_id('id_new_item')
        self.assertAlmostEqual(
            inputbox.location['x'] + inputbox.size['width']/2, size["width"]/2, delta=10
        )
